...
MatrixXdr  make_maf_ld_effect(double p_casual,double ld_ex,double maf_ex,double min_maf,double max_maf,double h2,int num){
...
int df_t_dist=4;
double alpha_factor= sqrt((1-h2A-h2nxe-h2gxe)*(df_t_dist-2)/df_t_dist);
boost::math::students_t_distribution<> dist_env_heavy_tail(df_t_dist);
boost::uniform_real<> std_uniform(0,1);
cout << (boost::math::quantile(dist_env_heavy_tail, 0.5)) << endl;
boost::variate_generator<boost::mt19937&, boost::uniform_real<> > effect_env_heavy_tail_helper(seedr, std_uniform);
...
for ( int i=0;i<num_simul;i++){
	...
	for (int j=0;j<Nindv;j++){
		...
		//  simul_pheno(j,i)=simul_pheno(j,i)+(Enviro(j,0)*effect_nxe())+effect_env();
                double effect_env_heavy_tail = boost::math::quantile(dist_env_heavy_tail, effect_env_heavy_tail_helper());
                simul_pheno(j,i)=simul_pheno(j,i)+(Enviro(j,0)*effect_nxe())+alpha_factor*effect_env_heavy_tail;
